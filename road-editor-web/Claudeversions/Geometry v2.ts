...
/**
 * Describes one neighboring road at a junction end.
 * `tangentAway` is the unit vector pointing FROM the shared node INTO the neighbor road.
 * `halfWidth` is the total half-width of the neighbor road at its end (lane + sidewalk on
 * the side that faces our road – used only for future advanced clipping, currently we
 * only need the tangent to build the miter plane).
 */
export interface NeighborInfo {
  tangentAway: THREE.Vector3; // unit vec, pointing away from junction node along neighbor
}

/**
 * Junction clipping info for one end of a road segment.
 * Pass `null` when the end is a dead-end (no neighbor).
 * Pass an array of NeighborInfo for every OTHER road connected at that node.
 */
export interface JunctionInfo {
  n1Neighbors: NeighborInfo[] | null; // neighbors at n1 end (other than this road)
  n2Neighbors: NeighborInfo[] | null; // neighbors at n2 end (other than this road)
}
...
export class RoadGeometry {
  ...
  // ---------------------------------------------------------------------------
  // JUNCTION CLIPPING HELPERS
  // ---------------------------------------------------------------------------

  /**
   * Given the set of neighbor tangents at one end of a road, compute the miter
   * plane normal that bisects this road's tangent and each neighbor's tangent.
   *
   * For a simple 2-road junction (one neighbor) the miter is the bisector of the
   * two road directions.  For T/+ intersections with multiple neighbors we use the
   * average bisector, which gives a reasonable flat cut.
   *
   * Returns the plane as { origin, normal } in 3-D.  The normal points INTO the
   * road (i.e. away from the junction node, along our road direction).
   *
   * @param ourTangentAway  unit vec pointing FROM node INTO our road
   * @param neighbors       other roads at this node
   */
  private static computeMiterPlane(
    nodePos: THREE.Vector3,
    ourTangentAway: THREE.Vector3,
    neighbors: NeighborInfo[]
  ): { origin: THREE.Vector3; normal: THREE.Vector3 } | null {
    if (neighbors.length === 0) return null;

    // Build a bisector direction for each neighbor pair and average them.
    // The miter bisector between two roads whose outgoing tangents are A and B
    // is simply normalize(A + B).  This is perpendicular to the "seam" line.
    let bisectorSum = new THREE.Vector3();
    for (const nb of neighbors) {
      // Both vectors point "away" from the node — their average bisects the gap.
      const bisector = ourTangentAway.clone().add(nb.tangentAway).normalize();
      bisectorSum.add(bisector);
    }

    if (bisectorSum.length() < 0.0001) {
      // Perfectly opposite roads — use the perp to our tangent as the cut plane
      bisectorSum = new THREE.Vector3()
        .crossVectors(ourTangentAway, new THREE.Vector3(0, 0, 1))
        .normalize();
    } else {
      bisectorSum.normalize();
    }

    // The miter PLANE normal is the bisector direction itself (it cuts across both roads).
    // Origin is at the node position.
    return { origin: nodePos.clone(), normal: bisectorSum };
  }

  /**
   * Intersect a 3-D ray (origin + direction) with a plane.
   * Returns the intersection point, or null if ray and plane are parallel.
   * `t` can be any real number — negative means the intersection is "behind" the ray origin.
   */
  private static rayPlaneIntersect(
    rayOrigin: THREE.Vector3,
    rayDir: THREE.Vector3,
    plane: { origin: THREE.Vector3; normal: THREE.Vector3 }
  ): THREE.Vector3 | null {
    const denom = plane.normal.dot(rayDir);
    if (Math.abs(denom) < 0.0001) return null; // parallel
    const t = plane.normal.dot(plane.origin.clone().sub(rayOrigin)) / denom;
    return rayOrigin.clone().add(rayDir.clone().multiplyScalar(t));
  }

  /**
   * Apply junction clipping to the first and/or last cross-section of the
   * already-computed finalEdges array.
   *
   * Strategy (your intuition, implemented correctly):
   *   1. Compute the bisector angle between the two meeting roads → the cut plane.
   *   2. Find how far the CENTER rail needs to move along the road tangent to sit
   *      exactly on that plane (scalar `t`).
   *   3. Apply that SAME scalar shift to every other rail (l_lane, r_lane, l_sw, r_sw),
   *      moving them all parallel to the road tangent by the same amount.
   *
   * Because all rails are parallel cross-section slices, a single longitudinal shift `t`
   * correctly clips the entire cross-section to the angled cut — no per-rail ray needed,
   * no deformed directions, no spikes.
   *
   * This modifies the array in-place.
   */
  private static applyJunctionClip(
    finalEdges: any[],
    junctionInfo: JunctionInfo,
    n1Pos: THREE.Vector3,
    n2Pos: THREE.Vector3
  ): void {
    const rails = ['center', 'l_lane', 'r_lane', 'l_sw', 'r_sw'] as const;
    const len = finalEdges.length;
    if (len < 2) return;

    // --- n1 end (index 0) ---
    if (junctionInfo.n1Neighbors && junctionInfo.n1Neighbors.length > 0) {
      // Road tangent pointing INTO the road from n1 (away from junction)
      const roadTangent = (finalEdges[1].center as THREE.Vector3)
        .clone()
        .sub(finalEdges[0].center as THREE.Vector3)
        .normalize();

      const plane = this.computeMiterPlane(n1Pos, roadTangent, junctionInfo.n1Neighbors);
      if (plane) {
        // Compute `t`: how far to move the center point along roadTangent to hit the plane.
        // Ray origin = current center at index 0, direction = roadTangent (into the road).
        const centerPt: THREE.Vector3 = (finalEdges[0].center as THREE.Vector3).clone();
        const hit = this.rayPlaneIntersect(centerPt, roadTangent, plane);
        if (hit) {
          // t is the signed distance along roadTangent from old center to hit point
          const shift = hit.clone().sub(centerPt); // vector along roadTangent
          // Apply the same shift to every rail
          for (const rail of rails) {
            const pt: THREE.Vector3 = (finalEdges[0] as any)[rail];
            const newPt = pt.clone().add(shift);
            newPt.z = pt.z; // preserve elevation
            (finalEdges[0] as any)[rail] = newPt;
          }
        }
      }
    }
    ...
    // --- n2 end (index len-1) ---
    if (junctionInfo.n2Neighbors && junctionInfo.n2Neighbors.length > 0) {
      // Road tangent pointing INTO the road from n2 (away from junction, so reversed)
      const roadTangent = (finalEdges[len - 2].center as THREE.Vector3)
        .clone()
        .sub(finalEdges[len - 1].center as THREE.Vector3)
        .normalize();

      const plane = this.computeMiterPlane(n2Pos, roadTangent, junctionInfo.n2Neighbors);
      if (plane) {
        const centerPt: THREE.Vector3 = (finalEdges[len - 1].center as THREE.Vector3).clone();
        const hit = this.rayPlaneIntersect(centerPt, roadTangent, plane);
        if (hit) {
          const shift = hit.clone().sub(centerPt);
          for (const rail of rails) {
            const pt: THREE.Vector3 = (finalEdges[len - 1] as any)[rail];
            const newPt = pt.clone().add(shift);
            newPt.z = pt.z;
            (finalEdges[len - 1] as any)[rail] = newPt;
          }
        }
      }
    }
  ...
  static calculateAllEdges(allData: PathPoint[], junctionInfo?: JunctionInfo, n1Pos?: THREE.Vector3, n2Pos?: THREE.Vector3): any[] {
    const rawEdges: any[] = [];
    ...
    // 4. Apply junction clipping at road ends (new logic)
    if (junctionInfo && n1Pos && n2Pos) {
      this.applyJunctionClip(finalEdges, junctionInfo, n1Pos, n2Pos);
    }
