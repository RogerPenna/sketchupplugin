...
export class RoadGeometry {
  ...
  /**
   * Clips a single 3-D point along the road's longitudinal direction so that it
   * lies exactly on the miter plane.
   *
   * Given a point `pt` and the road tangent direction `roadDir` (pointing away from
   * the junction), we slide `pt` along `roadDir` until it touches the plane.
   *
   * If the plane and direction are nearly parallel (grazing angle) we leave the
   * point unchanged.
   */
  private static clipPointToPlane(
    pt: THREE.Vector3,
    roadDir: THREE.Vector3,
    plane: { origin: THREE.Vector3; normal: THREE.Vector3 }
  ): THREE.Vector3 {
    const denom = plane.normal.dot(roadDir);
    if (Math.abs(denom) < 0.001) return pt.clone(); // nearly parallel – skip

    const toOrigin = plane.origin.clone().sub(pt);
    const t = plane.normal.dot(toOrigin) / denom;

    // Only move the point if it's on the wrong side (t > 0 means inside the road,
    // which is correct — we're extending/clipping toward the plane).
    // Allow a small tolerance so we don't push a point that's already past the plane.
    return pt.clone().add(roadDir.clone().multiplyScalar(t));
  }
  ...
  private static applyJunctionClip(
    ...
    if (junctionInfo.n1Neighbors && junctionInfo.n1Neighbors.length > 0) {
      // Our road's tangent AT n1, pointing INTO the road (away from n1)
      const ourTangent = (finalEdges[1].center as THREE.Vector3)
        .clone()
        .sub(finalEdges[0].center as THREE.Vector3)
        .normalize();

      const plane = this.computeMiterPlane(n1Pos, ourTangent, junctionInfo.n1Neighbors);
      if (plane) {
        for (const rail of rails) {
          const pt: THREE.Vector3 = finalEdges[0][rail];
          finalEdges[0][rail] = this.clipPointToPlane(pt, ourTangent, plane);
        }
      }
    }
    ...
    if (junctionInfo.n2Neighbors && junctionInfo.n2Neighbors.length > 0) {
      // Our road's tangent AT n2, pointing INTO the road (away from n2) — so reversed
      const ourTangent = (finalEdges[len - 2].center as THREE.Vector3)
        .clone()
        .sub(finalEdges[len - 1].center as THREE.Vector3)
        .normalize();

      const plane = this.computeMiterPlane(n2Pos, ourTangent, junctionInfo.n2Neighbors);
      if (plane) {
        for (const rail of rails) {
          const pt: THREE.Vector3 = finalEdges[len - 1][rail];
          finalEdges[len - 1][rail] = this.clipPointToPlane(pt, ourTangent, plane);
        }
      }
    }
  }
