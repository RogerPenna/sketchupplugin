...
function DragHandle({ direction, color, nodePos, onUpdate, onStart, onEnd, onSelect }: { 
  ...
  const handlePointerDown = (e: any) => {
    ...
    onSelect(); // Force selection on start
    onStart();
  ...
  const handlePointerMove = (e: any) => {
    ...
    if (raycaster.ray.intersectPlane(plane.current, currentIntersect)) {
      ...
      if (Math.abs(direction.z) > 0.5) {
        newPos.z += totalDelta.z;
      } else {
        newPos.x += totalDelta.x;
        newPos.y += totalDelta.y;
      }
      onUpdate(newPos);
    }
  };
...
function Node({ data, onChange, onSelect, isSelected, orbitControlsRef }: { 
  data: NodeData, 
  onChange: (newData: NodeData) => void,
  onSelect: () => void,
  isSelected: boolean,
  ...
  const handleUpdate = (newPos: THREE.Vector3) => {
    onChange({
      ...data,
      pos: newPos,
      left_h: newPos.clone().add(new THREE.Vector3(-5,0,0)), 
      right_h: newPos.clone().add(new THREE.Vector3(5,0,0))
    });
  };

  const toggleOrbit = (active: boolean) => {
    if (orbitControlsRef.current) orbitControlsRef.current.enabled = !active;
  };
...
function App() {
  ...
  const handleGroundClick = (e: any) => {
    if (!isAddingNode) { setSelectedIdx(null); setSelectedSegIdx(null); return; }
    e.stopPropagation();
  ...
  const deleteNode = () => {
    ...
    const next = nodes.filter((_, i) => i !== selectedIdx);
    setNodes(next);
    setSelectedIdx(null);
  };
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative', color: '#333', background: 'white' }}>
      ...
      <div style={{ position: 'absolute', top: 20, left: 20, zIndex: 10, background: 'rgba(255,255,255,0.95)', padding: '20px', borderRadius: '12px', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', width: '260px', border: '1px solid #eee' }}>
        <h2 style={{ margin: '0 0 5px 0', fontSize: '1.4rem', color: '#000', fontWeight: 800 }}>ROAD EDITOR</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
          <button style={{ padding: '10px', background: isAddingNode ? '#4CAF50' : '#222', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }} onClick={() => { setIsAddingNode(!isAddingNode); setSelectedIdx(null); setSelectedSegIdx(null); }}>{isAddingNode ? 'CLICK ON GROUND' : '+ Add Node Tool'}</button>
